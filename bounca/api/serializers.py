...
class CertificateSerializer(serializers.ModelSerializer):
    ...
    passphrase_in           = serializers.CharField(max_length=200, required=False)
    passphrase_out          = serializers.CharField(max_length=200, required=False)
    passphrase_out_confirmation  = serializers.CharField(max_length=200, required=False)

    class Meta:
        fields = ('id','shortname','name','parent','type','dn','created_at','expires_at','days_valid','revoked','crl_distribution_url','ocsp_distribution_host','passphrase_in','passphrase_out','passphrase_out_confirmation')
        model = Certificate
