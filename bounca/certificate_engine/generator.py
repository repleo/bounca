...
def generate_key(
    ...
    generate_ca_key_script = loader.render_to_string(
        generate_key_template_name, c)
    ...
    returncode = subprocess.check_output(
        [root_path + 'generate_%s_key.sh' % (key_name)])
...
def generate_cert(
    ...
    generate_ca_cert_script = loader.render_to_string(
        generate_cert_template_name, c)
    ...
    subprocess.check_output([root_path + "generate_%s_cert.sh" %
                             (key_name), str(certificate.days_valid), certificate.dn.subj])
...
def generate_csr(
    ...
    generate_csr_script = loader.render_to_string(
        generate_csr_template_name, c)
    with open(root_path + 'generate_csr.sh', 'w') as f:
        f.write(generate_csr_script)
    os.chmod(root_path + 'generate_csr.sh', 0o755)

    subprocess.check_output(
        [root_path + "generate_csr.sh", certificate.dn.subj])
    return 0
...
def sign_cert(
    ...
    sign_certificate_script = loader.render_to_string(
        sign_cert_template_name, c)
    ...
    subprocess.check_output(
        [root_path + "sign_certificate.sh", str(certificate.days_valid)])
    return 0


@generate_key_path
def generate_server_cert_creation_script(
        certificate,
        generate_signed_cert_template_name,
        key_path='.',
        root_path='.'):
    ...
    generate_signed_certificate_script = loader.render_to_string(
        generate_signed_cert_template_name, c)
...
@generate_key_path
def generate_client_cert_creation_script(
        certificate,
        generate_signed_cert_template_name,
        key_path='.',
        root_path='.'):
...
@generate_key_path
def generate_generic_cert_revoke_script(
        certificate,
        generate_cert_revoke_template_name,
        script_name,
        key_path='.',
        root_path='.'):
    ...
    generate_signed_certificate_script = loader.render_to_string(
        generate_cert_revoke_template_name, c)
...
@generate_key_path
def generate_generic_crl_file_script(
        certificate,
        generate_crl_file_template_name,
        key_path='.',
        root_path='.'):
    ...
    generate_signed_certificate_script = loader.render_to_string(
        generate_crl_file_template_name, c)
...
def generate_server_cert_revoke_script(
        certificate,
        generate_cert_revoke_template_name):
    return generate_generic_cert_revoke_script(
        certificate, generate_cert_revoke_template_name, "server_cert")


def generate_client_cert_revoke_script(
        certificate,
        generate_cert_revoke_template_name):
    return generate_generic_cert_revoke_script(
        certificate, generate_cert_revoke_template_name, "usr_cert")


@generate_key_path
def generate_certificate_info_script(
        certificate,
        generate_certificate_info_template_name,
        key_path='.',
        root_path='.'):
    ...
    generate_certificate_info_certificate_script = loader.render_to_string(
        generate_certificate_info_template_name, c)
...
@generate_key_path
def generate_test_passphrase_script(
        certificate,
        generate_test_passphrase_template_name,
        root_path='.',
        **kwargs):
    ...
    generate_test_passphrase_script = loader.render_to_string(
        generate_test_passphrase_template_name, c)
...
def generate_root_ca(certificate):
    ...
    generate_certificate_info_script(
        certificate, generate_cert_info_template_name)

    logger.info("Generate test_passphrase_key.sh")
    generate_test_passphrase_script(
        certificate, generate_test_passphrase_template_name)

    logger.warning("Create CRL File script")
    generate_generic_crl_file_script(
        certificate, generate_crl_file_template_name)
...
def revoke_client_cert(certificate, root_path='.', **kwargs):
    ...
    subprocess.check_output([root_path + "revoke_usr_cert_certificate.sh",
                             certificate.shortname, str(certificate.slug_revoked_at)])
    return 0
...
def get_certificate_info(certificate, key_path='.', root_path='.', **kwargs):
    ...
    out = subprocess.check_output(
        [root_path + "get_certificate_info.sh", path])
    return out
...
def generate_crl_file(certificate, key_path='.', root_path='.'):
    ...
    subprocess.check_output(
        [root_path + "generate_crl.sh", certificate.shortname])
    return 0
